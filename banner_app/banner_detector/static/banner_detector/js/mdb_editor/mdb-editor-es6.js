    ...
    return this.map((i, e) => {
      ...
      if (options.modalEditor === true) {
        ...
        const addNewRows = () => {
              ...
              let $newRow = [0];
              let standTypeId = $wrapperEditor.find('select[name="addStandType"] option:selected').val().trim();
              $newRow.push(standTypeId)
              for (let i = 0; i < editorOptions.headerLength; i++) {
                $value = $wrapperEditor.find('.addNewInputs input').eq(i).val()
                $newRow.push($value);
              }
              let numberInput = $newRow[2];
              let registrationNumber = $newRow[3];
            ...
            toggleDisabledToButtons = e => {
              ...
              if ($(e.target).parent().not('thead tr').not('tfoot tr').hasClass('tr-color-selected')) {
                ...
                $createShowP.html('1 Выбрано');
              } else if (!$('tr').hasClass('tr-color-selected')) {

                $buttonEdit.prop('disabled', true);
                $buttonDelete.prop('disabled', true);
                $createShowP.html('0 Выбрано');
              }
            },
            ...
            buttonEditInput = () => {
              // Fill id in editor
              $DataTable.row(
                  $wrapperEditor.find('.modalEditClass input[name="BusId"]').eq(0).val(
                      $DataTable.cell($selectedRow, 0).data()
                  )
              );
              console.log($DataTable.cell($selectedRow, 0).data());
              // Set selected value by name
              // $DataTable.row(
              //     $wrapperEditor.find('.modalEditClass select').eq(1).val(
              //         $DataTable.cell($selectedRow, 1).data()
              //     )
              // );
              for (let i = 0; i < editorOptions.headerLength - 2; i++) {
                $DataTable.row(
                    $wrapperEditor.find('.modalEditClass input[type="text"]').eq(i).val(
                        $DataTable.cell($selectedRow, i + 2).data()
                    )
                );
              }
            ...
            buttonEditInside = () => {
              let standTypeId = $wrapperEditor.find('select[name="editStandType"] option:selected').val().trim();
              let standTypeName = $wrapperEditor.find('select[name="editStandType"] option:selected').text();
              let busId = $wrapperEditor.find('.modalEditClass input[name="BusId"]').val();
              let busNumber = $wrapperEditor.find('.modalEditClass input[type="text"]').eq(0).val()
              let busRegistrationNumber = $wrapperEditor.find('.modalEditClass input[type="text"]').eq(1).val()

              $DataTable.cell($(trColorSelected).find('td').eq(0)).data(standTypeName);
              $DataTable.cell($(trColorSelected).find('td').eq(1)).data(busNumber);
              $DataTable.cell($(trColorSelected).find('td').eq(2)).data(busRegistrationNumber);
              $.ajax({
                ...
                success: function (data) {
                  if (data.bus.id) {
                    showAlert("Автобус изменен");
                  }
                }
              });


              // Send to SERVER standTypeId, busId, busNumber, busRegistrationNumber
            },
            ...
            buttonDeleteYes = () => {
              ...
              let busId = $DataTable.cell($selectedRow, 0).data()
              $.ajax({
                url: localStorage.getItem('bus-delete-link'),
                data: {
                  'busId': busId,
                  'csrfmiddlewaretoken': $('input[name="csrfmiddlewaretoken"]').val()
                },
                ...
                success: function (data) {
                  if (data.deleted) {
                    showAlert("Автобус успешно удалён");
                  }
                }
              });
